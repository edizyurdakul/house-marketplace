...
function Slider({ data }) {
  ...
  const [sliderRef, instanceRef] = useKeenSlider({
    ...
    // Fix Keenslider for slider per view, causing arrows to not gray out at end of slides
    slideChanged(slider) {
      if (windowDimensions.width >= 1280)
        setCurrentSlide(slider.track.details.rel + 3);
  ...
  useEffect(() => {
    // Reset slides to prev state to disable at start of slides
    if (currentSlide === 3 && windowDimensions.width >= 1280) {
      setCurrentSlide(0);
    }
    if (currentSlide === 2 && windowDimensions.width >= 768) {
      setCurrentSlide(0);
    }
    if (currentSlide === 1 && windowDimensions.width >= 640) {
      setCurrentSlide(0);
    }
  }, [currentSlide, windowDimensions]);
